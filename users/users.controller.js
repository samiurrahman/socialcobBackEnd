...
function authenticate(req, res, next) {
    userService.authenticate(req.body)
        .then(user => user ? res.json(user) : res.status(400).json({ message: 'Invalid credentials. Please try again.' }))
        .catch(err => next(err));
}
...
function singleUser(req, res, next) {
    userService.singleUser(req.body)
        .then((data) => data ? res.send(data) : res.status(400).json({ message: 'User Not Found. Please check Username.' }) )
        .catch(err => next(err));
}
